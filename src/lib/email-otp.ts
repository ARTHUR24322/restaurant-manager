...
/**
 * Envoie le code OTP par email via Gmail (Nodemailer)
 */
export async function sendOTPByEmail(
  ...
  try {
    ...
    const pass = process.env.GMAIL_APP_PASSWORD || process.env.ADMIN_PASSWORD;

    if (!user || !pass) {
      console.error("[OTP Email] GMAIL_USER ou GMAIL_APP_PASSWORD manquant dans les variables d'environnement.");
      return { success: false, error: "Configuration Gmail SMTP manquante." };
    }
    ...
    return { success: false, error: err?.message || "Erreur d'envoi email." };
  }
}
